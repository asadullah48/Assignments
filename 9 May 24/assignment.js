...
function genDynNum(x) {
    let randomNum = Math.floor(Math.random() * x) + 1;
    return randomNum % 2 === 0 ? randomNum : genDynNum(x);
}
const userInput = await inquirer.prompt([
    {
        message: 'Enter a valid number between 0 - 30 to start the game',
        name: 'nums',
        type: 'number',
    }
]);
let dynamicValue = genDynNum(dynamicNumber);
...
if (isNaN(userInput.nums)) {
    console.log(`Please, enter a valid number between 2 to 30.\n`);
}
...
let friends = ['usman', 'mansoor', 'mohsin', 'ifrahim', 'Sultan'];
...
let y = 8;
let z = '';
function calculator(x, z, y) {
    if (z === '+')
        return x + y;
    if (z === '-')
        return x - y;
    else
        console.log('Enter a valid number and choose either "+" or "-".');
}
